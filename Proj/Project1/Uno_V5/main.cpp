...
#include "Hands.h"   //Hand and Card Structures
...
void showHnd(vector<Card> *cards){
    cout<<"Enter the # that corresponds to your choice:"<<endl;
...
void play(){
    ...
    //Allocate Memory for array of player scores
    Scores *scores=new Scores[players];
    GameScrs *gameScrs=new GameScrs[players];
    ...
    for(int i=0; i<players; i++){           //Loop on # of players
        ...
        ////////////////////COMEBACKTOTHIS///////////////////////////
       // records->scores[i]->player=name;    //Store name in scores record   
    }
    ...
    do{//Main Game Loop
        do{//Player Turn Loop
            ...
            cout<<"added "<<hands[curPlyr].data.size()<<" to hand sizes"<<endl;
...
/*****************************************************************************/
/*                               SCORES                                       */
Scores *calcScrs(Hands *hands, int players, Scores *scores){


    int score=0;

    for(int p=0; p<players; p++){//Loop on players
...
/*
unsigned short lrgHnd(Hands *hands){
    unsigned short max=0;     //Set max to 0 
    for(int i=0; i<hands->hndSzs->size(); i++){//Get the Maximum Value
        if(max<hands->hndSzs->at(i)){
            max=hands->hndSzs->at(i);//Swap with higher value
        }
    }
    return max;
}
...
void *fillScr(Scores *scores, Hands *hands, int players){
    for(int i=0; i<players; i++){
        scores[i].lrgHnd=lrgHnd(hands);
