...
#include "Player.h"   //Hand and Card Structures
   ...
   //Start Menu
    ...
    do{//Main Program Loop
        do{
            ...
            if(strtChc=='1'||strtChc=='2'||strtChc=='Q') //Input Validation
                error=false;
            else{
                error=true;
...
void getPlyr(Player *hands, Scores *scores, int players, long &numRcrds){
    ...
    char name[25];
    //Get # of records
    in.open("scores.dat", ios::in|ios::out|ios::binary);
    if(in){ //If file exists
        in.read(reinterpret_cast<char *>(&get),sizeof(get));
        while(!in.eof()){
            numRcrds++;
            in.read(reinterpret_cast<char *>(&get),sizeof(get));
        }
    ...
    for(int p=0; p<players; p++){
        char choice;
        cout<<"Is Player "<<p+1<<" An Existing Player?"<<endl;
        cout<<"(Enter 'e' for existing player or any key for new player)"<<endl;
        cout<<"Enter e for existing or n for new"<<endl;
        cin>>choice;
        if(choice=='e'){ //Existing Player
            //
            cout<<"These are the existing players: "<<endl;
            wrtText();
            cout<<"Enter Your Player Number: ";
            cin>>rcrdNum;
            in.open("scores.dat", ios::in|ios::out|ios::binary);
            in.read(reinterpret_cast<char *>(&get),sizeof(get));
            while(!in.eof()){
                if(get.plyrNum==rcrdNum){
                    cout<<"Player Retrieved!"<<endl;
                    //Store player info
                    scores[p].allScrs=get.allScrs;
                    scores[p].avgScr=get.avgScr;
                    scores[p].highHnd=get.highHnd;
                    scores[p].nGames=get.nGames;
                    scores[p].nLoses=get.nLoses;
                    scores[p].nWins=get.nWins;
                    scores[p].plyrNum=get.plyrNum;
                    
                    cout<<"Player: "<<scores[p].name<<endl;
                    cout<<"Games Played: "<<scores[p].nGames<<endl;
                    cout<<"# of Wins: "<<scores[p].nWins<<endl;
                    cout<<"avg score: "<<scores[p].avgScr<<endl;
                    //Assign Player Info to In-Game Player Structure
                    hands[p].player=scores[p].name;
                }
                in.read(reinterpret_cast<char *>(&get),sizeof(get));
            }
            in.close();
            ...
            cin>>name;              //Store name in player's hand
            cin.clear();
            numRcrds++;
            plyrNum=numRcrds;
            cout<<"PLAYER #: "<<plyrNum<<endl;
            //store player name
            for(int i=0; i<25;i++){
                scores[p].name[i]=name[i];
            }
            hands[p].player=name;
            scores[p].plyrNum=plyrNum;
        }
    }
...
void play(){
    ...
    do{//Main Game Loop
        do{//Player Turn Loop
            hands[curPlyr].scores.hndSzs.push_back(hands[curPlyr].data.size());
            ...
            cout<<hands[curPlyr].player<<"'s HAND:"<<endl;
            showHnd(handPtr);
            ...
            do{//Play Card Loop
               ...
               if(canPlay){//Play A Card
                ...
                if(hands[curPlyr].data.size()==0){
                    //Store the winner
                    hands[curPlyr].scores.winLoose=true;
                    ...
                    cout<<hands[curPlyr].scores.gmScr<<endl;
                    cout<<"+***************************************+"<<endl;
                    //End Game
                    endgame=true;
                }
...
void prcCard(vector<Card> *cards, vector<Card> &draw, Card &discard){
    ...
    switch(discard.type){ //Switch on card type
        ...
        case(WILD):     //Wild
            ...
            if(colChc=='r') discard.colName="red";
            else if(colChc=='g') discard.colName="green";
            else if(colChc=='b') discard.colName="blue";
            else if(colChc=='y') discard.colName="yellow";
             break;
    ...
    for(int p=0; p<players; p++){
        if(p!=curPlyr){
            for(int h=0; h<hands[p].data.size(); h++){ //Loop on hand size
                score+=hands[p].data[h].points;
            }
        }
    }
    hands[curPlyr].scores.gmScr=score;
}
...
void lrgHnd(Player *hands, int players){
    ...
    for(int p=0; p<players; p++){
        max=0;
        int size=hands[p].scores.hndSzs.size();
        for(int h=0; h<size; h++){//Get the highest hand size
            if(max<hands[p].scores.hndSzs[h]){
                max=hands[p].scores.hndSzs[h];//Swap with higher value
            }
            hands[p].scores.lrgHnd=max;
        }
    }
}

void wrtBin(Player *hands, int players){
    fstream out; 
    
    out.open("scores.dat",ios::out|ios::binary);
    
    for(int i=0; i<players; i++){
        out.write(reinterpret_cast<char *>(&players[i]),sizeof(Player));
    }
}


void scores(Player *hands, int players){
