...
#include "hPlayer.h"//Human player Class
#include "cPlayer.h"//Computer player Class
...
int main(int argc, char** argv) {
 ...
 //  rdScr();
    ...
    do{//Main Game Loop
            ...
            if(typeid(*(plyrPtr))==typeid(Hplayer)){ //Ask for uno choice
                //Display message, ask if player wants to call uno
                cout<<"Enter u To Call UNO or any other key to continue: "<<endl;
                //Get uno choice
                cin>>unoChc;
            }
            else{ //Bot always calls uno
                if(plyrPtr->getUno()){
                    unoChc='u';
                }
                else{
                    cout<<plyrPtr->getName()<<" doesn't call uno."<<endl;
                }
            }
            //Uno actions
            ...
            do{//Play Card Loop
                ...
                if(cPlay){  //Play A Card
                    ...
                    do{
                        bool validate=false;
                        if(typeid(*(plyrPtr))==typeid(Hplayer)){
                            cout<<"\nNo Valid Card to Play!"<<endl;
                            cout<<"Enter any key to continue"<<endl;
                            cin>>cont;
                        }
                        else{
                            cout<<plyrPtr->getName()<<" has no valid card to play!"<<endl;
                        }
                        //Draw A Card
...
void prcCard(Player *plyr,Deck &deck, Card &discard){
    ...
    switch(discard.getType()){ 
        ...
        case(WILD)://Wild
            do{//Input Validation
                if(typeid(*(plyr))==typeid(Hplayer)){ //if human
